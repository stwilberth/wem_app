...
class SegundoCtrl extends Component
{
    public ?FormSegundo $FormSegundo = null;
    ...
    public function mount() {
        ...
        if ($this->Hombre) {
            if ($this->Hombre->total_sesiones >= 15 && $this->Hombre->total_sesiones <= 20) {
                if ($this->Hombre->form_segundo == 'sin_iniciar') {
                    $this->FormSegundo ??= new FormSegundo();
                    $this->FormSegundo->hombre_id = $this->hombre_id;
                    $this->Hombre->form_segundo = 'editando';
                    $this->FormSegundo->save();
                    $this->Hombre->save();
                } else if ($this->Hombre->form_segundo == 'editando') {
    ...
    {
        return view('livewire.forms.segundo')
            ->extends('layouts.app')
            ->section('content')
            ->layoutData([
